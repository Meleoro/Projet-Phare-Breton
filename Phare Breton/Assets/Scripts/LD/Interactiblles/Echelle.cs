...
public class Echelle : ObjetInteractible
{
    ...
    public Vector3 FindTPPos(Transform posChara)
    {
        float angle = 0;
        pointsGround.Clear();
        ...
        if (posChara.position.y > center.position.y)
        {
            for (int i = 0; i < 8; i++)
            {
                ...
                Vector3 posRaycast = TPPosBas.position + new Vector3(direction.x, 0.5f, direction.y);
        ...
        else
        {
            for (int i = 0; i < 8; i++)
            {
                ...
                Vector3 posRaycast = TPPosHaut.position + new Vector3(direction.x, 0.5f, direction.y);
    ...
    {
        chara.position = FindTPPos(chara);
    }
